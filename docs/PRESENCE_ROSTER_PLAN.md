...
Selected by a per-account resolver (see §4). `:poll` is the safe default.

| Mode | Latency | Visibility enforcement | Cost / transition | Use when |
|------|---------|------------------------|-------------------|----------|
| **`:poll`** | ~poll interval (e.g. 5–30s) | Server-side query (`WHERE IN visible_scope`) — **any** rule, incl. asymmetric | Flat: 1 cached query per heartbeat/poll per session | Latency tolerable; arbitrary visibility; simplest & safest |
| **`:nudge`** | Near-instant | Server-side query (same as poll) | O(1) content-free broadcast → each viewer filtered-polls | Low latency **and** restricted visibility (the usual restricted answer) |
...
Notes:
- `:poll` and `:nudge` filter identically (server query); `:nudge` just adds an
...
- **Axis 1 — latency need (consumer's call):** acceptable → `:poll`; low → a push
  mode.
- **Axis 2 — visibility (only for push):** open account → `:broadcast`;
  restricted account → `:nudge` (default) or `:fanout` (timing-sensitive).

`:poll` needs no visibility sub-switch because it filters server-side regardless.
...
- **Wire message contracts** (identical across modes so the client reducer never
  ...
  - client → server action: `poll` (poll/nudge modes)
...
WhereIsWaldo.configure do |config|
  ...
  # What a viewer MAY SEE (for poll/nudge/fanout snapshots + poll diffs).
  # AR scope of subjects. Defaults to roster_org's members if unset.
  config.roster_visible_to = ->(viewer) { viewer.visible_users }

  # Who may SEE a subject (ONLY needed for :fanout). AR scope of viewers.
  # Must be the exact inverse of roster_visible_to.
...
2. `roster_viewers_of` and `roster_visible_to` must be exact inverses
...
4. Default mode is the **safe** one (`:poll`); fast/leaky paths are opt-in.
...
module WhereIsWaldo::RosterDelivery
  class Base
    ...
    def on_poll(channel, subject); end         # RosterChannel#poll (poll/nudge)
...
- **`Broadcast`** — `on_subscribe`: `stream_from account_stream`, transmit
  snapshot (full `roster_members`, or `visible_scope` when a client filter is
  advertised). `on_transition`: broadcast `roster_delta` to `account_stream`.
  `on_poll`: n/a.
- **`Poll`** — `on_subscribe`: transmit initial snapshot (`visible_scope`),
...
- **`Nudge < Poll`** — same as Poll, plus `on_transition`: broadcast
...
- (poll uses no stream — data rides the `poll` transmit)
...
- `:poll` / `:nudge`: run a jittered interval calling `perform('poll')`.
...
|------|----------------|-------------------------------|
| `:poll` / `:nudge` / `:fanout` | Server (query / audience) | No |
...
(`roster_org` + shared stream + `usePresenceRoster` + specs). Migration:
...
- No DB migration required (poll uses cache-map diffs, not tombstones).
...
- **Mode resolver**: symbol vs callable; per-account uniformity; default `:poll`.
- **Broadcast**: shared-stream subscribe + transmit; transition → account delta;
  client filter cosmetic (reducer-level).
- **Poll**: cache miss → snapshot; hit → delta; offline via diff; stale → resync;
...
- **Phase 1 — foundation + the two anchors.** Strategy abstraction + resolver;
  `:broadcast` (refactor of existing) + `:poll` (cache-diff). Covers "open
  account instant" and "acceptable-latency arbitrary visibility." Ship + version
  bump.
- **Phase 2 — low-latency restricted.** `:nudge` (Poll + content-free trigger,
  jitter). Ship + bump.
...
4. **Config key names** — `roster_org` / `roster_visible_to` /
   `roster_viewers_of` / `roster_mode` — final?
5. **Client filter**: flag on `:broadcast`, or its own mode name for louder
   intent (e.g. `:broadcast_client_filtered`)?
6. **Default mode** confirm `:poll` (safe) vs `:broadcast` (back-comptible with
   what's built).
